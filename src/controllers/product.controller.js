...
const getProductById = async (req, res, next) => {
    ...
    const product = await Product.findById(_id, { createdAt: 0, updatedAt: 0, __v: 0 }).populate('category', 'name').lean()
...
const searchProduct = async (req, res, next) => {
    ...
    const products = await Product.find(
        ...
        { description: 0, size: 0, color: 0, createdAt: 0, updatedAt: 0, __v: 0 }
    ).populate('category', 'name')
...
const suggestProduct = async (req, res, next) => {
    ...
    const products = await Product.find(
        {},
        { description: 0, size: 0, color: 0, createdAt: 0, updatedAt: 0, __v: 0 }
    ).populate('category', 'name').limit(8).sort({ price: -1 })
...
const favoriteProduct = async (req, res, next) => {
    ...
    await User.updateOne(
        { _id: req.user._id },
        {
            ...(love === 'true' ? { $addToSet: { favorites: product } } : { $pull: { favorites: product } })
        }
    )
